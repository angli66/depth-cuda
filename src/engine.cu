...
py::array_t<uint8_t> compute(py::array_t<uint8_t> left_ndarray, py::array_t<uint8_t> right_ndarray) {
    ...
    Mat2d<uint8_t> depth = compute_depth_method(left, right);
    py::array_t<uint8_t> depth_ndarray = Mat2d2ndarray<uint8_t>(depth);
